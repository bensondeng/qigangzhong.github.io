...
>
> 2. 命名卷：通过`docker volume create VOLUME_NAME`命令创建的卷，实际路径在`/var/lib/docker/volumes`下面，通过这种方式创建出来的卷通过卷名就可以挂载，例如`docker run -v VOLUME_NAME:/containerdir IMAGE_NAME`
>
...
> CMD命令相当于是在`docker run`命令之后的默认参数, 如果`docker run`命令之后添加了其它参数，会覆盖掉CMD的内容
>
...
也可以通过dockerhub账号管理github账号，有提交时进行[自动构建](https://docker_practice.gitee.io/repository/dockerhub.html)
